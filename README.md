...
> 이번 PoC 범위에 포함하지 않았습니다.

## 아키텍처 요약

```
T점수 JSON --(규칙 기반 판정)--> classifier.py --(프롬프트 생성)--> LLM(Gemma3 27B)
...
| 항목 | 내용 |
|---|---|
| LLM | `gemma-3-27b-it` (Google AI Studio API, `google-genai` SDK) |
| 호출 방식 | 검사(요청)당 1회 `client.models.generate_content()` 호출 |
| 무료 티어 한도 | 분당/일별 요청 수 제한(RPM/RPD)이 있습니다. 데모 시 연속 호출을 피하고
호출 사이에 간격을 두세요 (짧은 시간 내 여러 케이스를 연달아 생성하면 429 오류가 날 수 있음). |
...
리포트가 화면에 렌더링됩니다. (`vite.config.ts`의 `server.proxy` 설정으로
`/report`, `/api` 요청이 자동으로 백엔드에 프록시됩니다.)
...
| `GEMINI_MODEL` | 사용할 모델명 (기본값 `gemma-3-27b-it`) | `backend/.env` (선택) |
...
│   │   ├── llm_client.py          # Google AI Studio API 연동
│   │   ├── guardrail.py           # 금지어 필터 + 면책 문구
│   │   ├── report_generator.py    # 파이프라인 조립
│   │   └── main.py                # FastAPI (POST /report)
│   ├── tests/test_classifier.py
│   ├── .env.example
│   └── requirements.txt
├── frontend/
│   ├── src/
│   │   ├── components/            # ReportForm, ReportView, StepZeroCard 등
│   │   ├── api/reportApi.ts
│   │   └── lib/types.ts
│   └── vite.config.ts
├── .claude/PRD.md
